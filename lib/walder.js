...
const defaultOptions = {port: 3000, logging: 'info', cache: true, cwd: process.cwd()};
...
module.exports = class Walder {
  constructor(configFile, options = {}) {
    ...
    if (!Path.isAbsolute(this.configFile)) {
      this.configFile = Path.resolve(this.cwd, configFile);
    }
  }
  ...
  async initialise() {
    ...
    for (let path in yamlData.paths) {
      ...
      for (let method in yamlData.paths[path]) {
        ...
        const graphQLLDInfo = GraphQLLDParser.parse(yamlData.paths[path][method]['x-walder-query'], dataSources, this.cache, parameters);
