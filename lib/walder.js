  ...
  async initialise() {
    ...
    for (let path in yamlData.paths) {
      ...
      for (let method in yamlData.paths[path]) {
        ...
        if (await validator.validate({routeInfo, parameters, graphQLLDInfo, htmlInfoDictionary})) {
    ...
    if (await validator.validate({routeInfo: new RouteInfo('default error pages', 'any'), htmlInfoDictionary: defaultErrorPages})) {
      Object.keys(validator.prevErrors).forEach((type) => {
        this.logger.error('%s:  %s', type, validator.prevErrors[type]);
      });
    }
