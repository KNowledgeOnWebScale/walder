  ...
  async initialise() {
    ...
    for (let path in yamlData.paths) {
      ...
      for (let method in yamlData.paths[path]) {
        ...
        if (validator.validate({routeInfo: routeInfo, parameters: parameters, graphQLLDInfo: graphQLLDInfo, htmlInfoDictionary: htmlInfoDictionary})) {
    ...
    if (validator.validate({routeInfo: new RouteInfo('default error pages', 'any'), htmlInfoDictionary: defaultErrorPages})) {
      Object.keys(validator.prevErrors).forEach((type) => {
        this.logger.error('%s:  %s', type, validator.prevErrors[type]);
      });
    }
