...
class RdfConverter extends Converter {
  ...
  convert(format, data, queryInfo) {
    ...
    } else if (queryInfo.jsonldFrame) {
      jsonld = data.data;
    } else {
      if (format === RDF_TYPES.NQ) {
        return data;
      }
      jsonld = JsonLD.fromRDF(data, {format: 'application/n-quads'});
    }

    if (format === RDF_TYPES.JSON_LD) {
      return jsonld;
    } else {
      return this._convertJSONLDToOther(jsonld, format);
    }
  }
  ...
  _convertNQuadsToOther(data, format) {
    return new Promise((resolve, reject) => {
      const writer = new N3.Writer({format});
      writer.addQuads(parser.parse(data));
