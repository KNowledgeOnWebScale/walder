  ...
  convert(htmlInfo, data) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        fileContent = fs.readFileSync(htmlInfo.file, 'utf8'); // For some reason the async version take a super long time.
      ...
      try {
        if (content.attributes.layout) {
          try {
            ...
            const html = await this.convert(new HTMLInfo(layoutEngine, layoutPath), layoutData);
