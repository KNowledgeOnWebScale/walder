  ...
  convert(htmlInfo, data, jsonld) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        content = this.templateLoader.getTemplateFromCache(htmlInfo);
      ...
      if (jsonld) {
        data._queryResultsAsJSONLD = JSON.stringify(jsonld);
      }
      ...
      try {
        if (content.attributes.layout) {
          try {
            ...
            if (this.logger) {
              this.logger.debug(`Generating HTML using layout "${layoutInfo.file}".`);
            }

            const html = await this.convert(layoutInfo, layoutData, jsonld);

            if (this.logger) {
              this.logger.debug(`Done generating HTML using layout "${layoutInfo.file}".`);
            }
