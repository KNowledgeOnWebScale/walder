...
module.exports = class HtmlConverter extends Converter {
  ...
  static convert(htmlInfo, data) {
    return new Promise(async (resolve, reject) => {
      ...
      // FrontMatter metadata as additional attributes in data
      ...
      try {
        if (content.attributes.layout) {
          try {
            ...
            // inner html content in data, object named 'content'
            // FrontMatter metadata in data, as attributes in an object named `attributes`
            const html = await HtmlConverter.convert(new HTMLInfo(layoutEngine, layoutPath), {content: layoutDataContent, attributes: content.attributes});
            resolve(html);
          } catch (err) {
            reject(err);
          }
