...
const queryEngineComunicaCache = {};
 ...
 * @param cache, boolean stating whether or not to use cached QueryEngineComunica's
 * @returns {Promise<>}, GraphQL-LD query results
 */
const executeQuery = async (comunicaConfig, graphQLLD, variables, queryParams, cache) => {
  ...
  if (cache) {
    if (! comunicaConfig.sources.toString() in queryEngineComunicaCache) {
      queryEngineComunicaCache[comunicaConfig.sources.toString()] = new QueryEngineComunica(comunicaConfig);
    }
    const queryEngine = queryEngineComunicaCache[comunicaConfig.sources.toString()];
