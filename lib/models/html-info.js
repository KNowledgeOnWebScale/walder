...
module.exports = class HTMLInfo {
  ...
  /**
   * Creates a new htmlInfo object for a given layout file that extends this template.
   *
   * @param layout - The layout file
   */
  getLayoutInfo(layout) {
    const layoutPath = path.join(this.layoutsDir, layout);
    const layoutEngine = getEngineFromFilePath(layoutPath);
    return new HTMLInfo(layoutEngine, layoutPath, this.description, this.layoutsDir)
  }
};
