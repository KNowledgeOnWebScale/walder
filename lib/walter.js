...
module.exports = class Walter {
  ...
  initialise() {
    ...
    const meta = new MetaParser(yamlData).parse();
    ...
    this.app.use(express.static(Path.resolve(meta.path, meta.public.path)));
    ...
    for (let path in yamlData.paths) {
      for (let method in yamlData.paths[path]) {
        ...
        const htmlInfo = htmlParser.parse(path, method, Path.resolve(meta.path, meta.views));


        // Load remote and local pipe modules
        const pipeFunctions = pipeModuleLoader.load(pipeModules, Path.resolve(meta.path, meta['pipe-modules']));
