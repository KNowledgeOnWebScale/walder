...
module.exports = class Walter {
  ...
  initialise() {
     ...
     * @returns {Function}
     */
    const sendResponseCallback = (res, contentType) => {
      return (html) => {
        res
     ...
     * @param htmlInfo, HTMLInfo object (see walter/lib/models/htmlInfo.js)
     * @param graphQLLD, GraphQLLD info object (see walter/lib/models/graphQLLDInfo.js)
     * @param req, express 'request' object
     * @param res, express 'response' object
     */
    const contentNegotiation = (data, htmlInfo, graphQLLD, req, res) => {
      const accept = accepts(req);
      const callback = sendResponseCallback(res, accept.type(CN_TYPES));


      // the order of this list is significant; should be server preferred order
      switch (accept.type(CN_TYPES)) {
        case 'text/html':
          HtmlConverter.convert(htmlInfo, data, callback);
          break;
