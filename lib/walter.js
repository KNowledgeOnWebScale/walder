...
module.exports = class Walter {
  ...
  initialise() {
    ...
    // Resources
    const resources = ResourceParser.parse(yamlData['x-walter-resources'], this.configFile);

    // Datasources
    const dataSources = yamlData['x-walter-datasources'];


    // Parse the default error pages section
    const defaultErrorPages = HtmlParser.parse(yamlData['x-walter-errors'], resources.views);
    ...
    for (let path in yamlData.paths) {
      ...
      for (let method in yamlData.paths[path]) {
        ...
        const graphQLLDInfo = GraphQLLDParser.parse(yamlData.paths[path][method]['x-walter-query'], dataSources, this.cache);
        ...
        const pipeModules = PipeModuleParser.parse(yamlData.paths[path][method]['x-walter-postprocessing'], resources['pipe-modules']);
