...
module.exports = class Walter {
  ...
  initialise() {
    ...
    const meta = new MetaParser(yamlData.meta).parse();
    console.log(meta);
    ...
    for (let path in yamlData.paths) {
      for (let method in yamlData.paths[path]) {
        ...
        const pipeFunctions = pipeModuleLoader.load(pipeModules, Path.resolve(meta.resources.path, meta.resources['pipe-modules']));
