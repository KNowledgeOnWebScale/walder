...
module.exports = class GraphQLLDHandler extends Handler {
  ...
  async handle(graphQLLDInfo, variables, queryParams) {
    ...
    if (graphQLLDInfo.cache) {
      ...
      if (!(graphQLLDInfo.comunicaConfig.sources.toString() in this.COMUNICA_ENGINE_CACHE)) {
        this.COMUNICA_ENGINE_CACHE[graphQLLDInfo.comunicaConfig.sources.toString()] = new QueryEngineComunica(graphQLLDInfo.comunicaConfig);
        logger.verbose('Comunica query engine cache: MISS');
      } else {
        logger.verbose('Comunica query engine cache: HIT');
      }
      clientConfig.queryEngine = this.COMUNICA_ENGINE_CACHE[graphQLLDInfo.comunicaConfig.sources.toString()];
