...
module.exports = class GraphQLLDHandler extends Handler {
  ...
  removeDuplicates(data,options) {
    ...
    data.forEach(result => {
      ...
      while(structure.value){
        comparator = comparator[structure.value];
        structure = structure.selector;
      }

      comparator = structure.selector ? comparator[structure.selector] : comparator[structure];
  ...
  sortBy(array, selectors) {
    return array.sort((a, b) => {
      for (let selector of selectors) {
        ...
        const objA = a[selector.selector] ? a[selector.selector].toUpperCase() : a[selector].toUpperCase();
        const objB = b[selector.selector] ? b[selector.selector].toUpperCase() : b[selector].toUpperCase();
