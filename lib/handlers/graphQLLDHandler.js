...
module.exports = class GraphQLLDHandler extends Handler {
  ...
  async handle(graphQLLDInfo, pathParams = {}, queryParams = {}) {
    const newQueries = Object.keys(graphQLLDInfo.queries).reduce((acc, val) => {
      ...
      acc[val] = {};
      acc[val].query = this._substituteQueryParams(this._substituteVariables(query, pathParams), queryParams);
   ...
   */
  _substituteVariables(query, variables) {
   ...
   * @returns string (newQuery)
   */
  _substituteQueryParams(query, params) {
    ...
    if (keys && keys.length > 0) {
      ...
      delete params.page;
      return this._substituteVariables(query, params);
    } else {
      return query;
    }
  };
