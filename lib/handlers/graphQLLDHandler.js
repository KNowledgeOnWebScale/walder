...
module.exports = class GraphQLLDHandler extends Handler {
  ...
  async handle(graphQLLDInfo, variables, queryParams) {
    let newQueries = graphQLLDInfo.queries.map(query => this.substituteVariables(query, variables));
    newQueries = newQueries.map(query => this.substituteQueryParams(query, queryParams));
