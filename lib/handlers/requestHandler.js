        ...
        if (graphQLLDInfo && Object.keys(graphQLLDInfo).length > 0) {
          this.graphQLLDHandler.handle(graphQLLDInfo, req.params, req.query)
            .then((data) => {
              ...
              try {
                const pipeResult = data.map(dataEntry => UTILS.pipe(pipeFunctions)(dataEntry).data);

                this.contentNegotiationHandler.handle(pipeResult.length === 1 ? pipeResult[0] : pipeResult, htmlInfo, graphQLLDInfo, req, res);
              } catch (e) {
