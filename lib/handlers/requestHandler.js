...
module.exports = class RequestHandler extends Handler {
  constructor() {
    super();
    this.graphQLLDHandler = new GraphQLLDHandler();
  ...
  handle(graphQLLDInfo, pipeFunctions, htmlInfo, status) {
    return (req, res, next) => {
      if (! isEmpty(graphQLLDInfo)) {
        this.graphQLLDHandler.handle(graphQLLDInfo, req.params, req.query)
          .then((data) => {
            ...
            try {
              ...
              this.contentNegotiationHandler.handle(pipeResult.data, htmlInfo, graphQLLDInfo, req, res);
            }
            catch (e) {
        ...
        HTMLConverter.convert(htmlInfo[String(status)], {}, this.responseHandler.handle(res, 404, 'text/html'));
      }
    }
  };
};