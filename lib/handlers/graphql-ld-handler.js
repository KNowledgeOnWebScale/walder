  ...
  async handle(graphQLLDInfo, pathParams = {}, queryParams = {}) {
    ...
    for (const key of Object.keys(newQueries)) {
      try {
        ...
        // });
        const result1 = await this.comunicaEngine.query(newQueries[key].query, {
          ...graphQLLDInfo.comunicaConfig,
          ...graphQLLDInfo.context,
          queryFormat: 'graphql'
        });
        const result2 = await bindingsStreamToGraphQl(result1.bindingsStream, result1.context, {materializeRdfJsTerms: true});
        results[key] = {data: result2};
      } catch (error) {
        if (this.logger) {
