  ...
  async handle(graphQLLDInfo, pathParams = {}, queryParams = {}) {
    ...
    for (const key of Object.keys(newQueries)) {
      try {
        ...
        // });
        const result = await this.comunicaEngine.query(newQueries[key].query, {
          ...graphQLLDInfo.comunicaConfig,
          ...graphQLLDInfo.context,
          queryFormat: 'graphql'
        });
        results[key] = await bindingsStreamToGraphQl(result.bindingsStream, result.context, {materializeRdfJsTerms: true});
      } catch (error) {
        if (this.logger) {
