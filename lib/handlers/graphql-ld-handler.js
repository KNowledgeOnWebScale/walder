...
module.exports = class GraphQLLDHandler extends Handler {
  ...
  async handle(graphQLLDInfo, pathParams = {}, queryParams = {}) {
    ...
    if (!(graphQLLDInfo.comunicaConfig.sources.toString() in this.comunicaEngineSourcesMap)) {
      graphQLLDInfo.comunicaConfig.sources = await parseDataSources(graphQLLDInfo.comunicaConfig.sources, self, this.pipeModulesPath);
      ...
      keys.forEach(key => {
        ...
        if (definedParameters[key].type === 'integer') {
          ...
          if (definedParameters[key].minimum !== undefined && intVal < definedParameters[key].minimum) {
            ...
            err.type = 'GRAPHQL-INTEGER-BELOW-MINIMUM';
            throw err;
          }
          ...
          if (definedParameters[key].maximum !== undefined && intVal > definedParameters[key].maximum) {
            ...
            err.type = 'GRAPHQL-INTEGER-ABOVE-MAXIMUM';
            throw err;
          }
