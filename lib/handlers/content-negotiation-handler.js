    ...
    try {
      switch (accept.type(CN_TYPES)) {
        case 'text/html':
          ...
          const materializedData = SPARQLJSONConverter.materializeTree(data);
          const html = await converter.convert(htmlInfo['200'], materializedData);
          callback(html);
          break;
        case 'application/ld+json':
          callback(await this.rdfConverter.convert(RDF_TYPES.JSON_LD, data, graphQLLD));
          break;
        case 'text/turtle':
          callback(await this.rdfConverter.convert(RDF_TYPES.TURTLE, data, graphQLLD));
          break;
        case 'application/n-triples':
          callback(await this.rdfConverter.convert(RDF_TYPES.NT, data, graphQLLD, callback));
          break;
        case 'application/n-quads':
          callback(await this.rdfConverter.convert(RDF_TYPES.NQ, data, graphQLLD, callback));
          break;
